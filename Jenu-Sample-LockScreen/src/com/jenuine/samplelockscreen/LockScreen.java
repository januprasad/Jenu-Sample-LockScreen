...
package com.jenuine.samplelockscreen;
...
/**
 * This is a copy of com.android.internal.widget.multiwaveview.LockScreen with
...
public class LockScreen extends View {
	...
	public LockScreen(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.LockScreen);
		mInnerRadius = a.getDimension(R.styleable.LockScreen_innerRadius, mInnerRadius);
		mOuterRadius = a.getDimension(R.styleable.LockScreen_outerRadius, mOuterRadius);
		mSnapMargin = a.getDimension(R.styleable.LockScreen_snapMargin, mSnapMargin);
		mVibrationDuration = a.getInt(R.styleable.LockScreen_vibrationDuration, mVibrationDuration);
		mFeedbackCount = a.getInt(R.styleable.LockScreen_feedbackCount, mFeedbackCount);
		TypedValue handle = a.peekValue(R.styleable.LockScreen_handleDrawable);
		mHandleDrawable = new TargetDrawable(res, handle != null ? handle.resourceId : 0, 2);
		mHandleDrawable.setState(TargetDrawable.STATE_INACTIVE);
		mOuterRing = new TargetDrawable(res, getResourceId(a, R.styleable.LockScreen_outerRingDrawable), 1);

		mAlwaysTrackFinger = a.getBoolean(R.styleable.LockScreen_alwaysTrackFinger, false);

		int pointId = getResourceId(a, R.styleable.LockScreen_pointDrawable);
		Drawable pointDrawable = pointId != 0 ? res.getDrawable(pointId) : null;
		mGlowRadius = a.getDimension(R.styleable.LockScreen_glowRadius, 0.0f);

		TypedValue outValue = new TypedValue();

		// Read array of target drawables
		if (a.getValue(R.styleable.LockScreen_targetDrawables, outValue)) {
		...
		if (a.getValue(R.styleable.LockScreen_targetDescriptions, outValue)) {
		...
		if (a.getValue(R.styleable.LockScreen_directionDescriptions, outValue)) {
			final int resourceId = outValue.resourceId;
			if (resourceId == 0) {
		...
		mGravity = a.getInt(R.styleable.LockScreen_android_gravity, Gravity.TOP);
		a.recycle();
